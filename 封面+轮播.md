...
## 功能使用[](id:moreFeature)
### 1、视频封面
超级播放器支持用户自定义视频封面，用于在视频开始播放前展示。视频封面支持使用网络url地址 或者本地FileID。
- 若视频打开后自动播放，则在视频首帧加载出来之前展示封面。
- 若视频打开后未自动播放，则将在此时即展示视频封面，直到用户点击开始播放，视频首帧加载出来，均展示视频封面。
...
### 2、列表轮播
超级播放器支持视频列表轮播，即在给定一个视频列表后，支持按顺序循环播放列表中的视频，列表中最后一个视频播放完成后将自动开始播放列表中的第一个视频。
